...
CameraQuat::CameraQuat(int w, int h, const Vec3f& pos, const Vec3f& target, const Vec3f& up):
    _pos(pos), 
    _target(target), 
    _up(up), 
    _winWidth(w), _winHeight(h),
...
bool CameraQuat::onKeyboard(int key) {
    bool ret = false;
...
void CameraQuat::setPos(const Vec3f& pos) {
    _pos = pos;
}

void CameraQuat::setTarget(const Vec3f& target) {
    _target = target;
}

void CameraQuat::setUp(const Vec3f& up) {
    _up = up;
}

const Vec3f& CameraQuat::getPos() const {
    return _pos;
}

const Vec3f& CameraQuat::getTarget() const {
    return _target;
}

const Vec3f& CameraQuat::getUp() const {
    return _up;
}

void CameraQuat::init() {
