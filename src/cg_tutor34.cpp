...
static Shader * pNormalShader = NULL;
// 贴图
static Texture * pTextureCube = NULL;
...
static void renderModel(float t) {
    ...
    Mat4f model;
    model = Mat4f::scale(model, Vec3f(1.0f));
    ...
    pTextureCube->apply(GL_TEXTURE0);
    pModel -> render(pShader);

    pNormalShader -> use();
    pNormalShader -> setMat4f("projection", projection);
    pNormalShader -> setMat4f("view", view);
    pNormalShader -> setMat4f("model", model);
    pModel -> render(pNormalShader);
}
...
static void initTextures() {
    pTextureCube = new Texture("textures/test.png", Texture::Diffuse);
}

/* 初始化着色器 */
static void initShaders() {
    pShader = new Shader(
        "GLSL/vs34.default.glsl", 
        "GLSL/fs34.default.glsl"
    );
    pNormalShader = new Shader(
        "GLSL/vs34.normal_visualization.glsl",
        "GLSL/fs34.normal_visualization.glsl",
        "GLSL/gs34.normal_visualization.glsl"
    );
}
...
void testOGLTutorial() {
    glw::openGlWindow(
        renderSceneCB, 
        "OpenGL Tutorial 34 - Geometry Shader: Exploding Objects", 
        init, 
        reshape, 
        renderSceneCB,
        true, 
