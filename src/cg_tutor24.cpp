...
// 灯光方向
static Vec3f lightPos(3.0f, 2.0f, -1.0f);
// 灯光颜色
...
static void renderSceneCB() {
    ...
    pLightingShader -> use();
    pLightingShader -> setVec3f("light.position", pCamera -> getPosition());
    pLightingShader -> setVec3f("light.direction", pCamera -> getFront());
    pLightingShader -> setFloat("light.cutOff", cosf(toRadian(12.5f)));
    pLightingShader -> setFloat("light.outerCutOff", cosf(toRadian(15.0f)));
...
static void initShaders() {
    pLightingShader = new Shader("GLSL/vs24.spotLight.glsl", "GLSL/fs24.spotLight.glsl");
...
static void initLight() {
    pLightingShader -> use();
    // 环境光和漫反射
    Vec3f diffuseColor = lightColor * Vec3f(1.0f);
    Vec3f ambientColor = lightColor * Vec3f(0.1f);   
    ...
    // 衰减
    pLightingShader -> setFloat("light.constant", 1.0f);
    pLightingShader -> setFloat("light.linear", 0.07f);
    pLightingShader -> setFloat("light.quadratic", 0.017f);
}
